...
databases = []
connection_string_template = r"DRIVER={{ODBC Driver 17 for SQL Server}};SERVER={};DATABASE={};UID={};PWD={};"
csv_directory = ''

def get_db_connection(server, database, username, password):
    connection_string = connection_string_template.format(server, database, username, password)
...
def handle_data(server, database, selected_tables, action, username, password):
    try:
        conn = get_db_connection(server, database, username, password)
        cursor = conn.cursor()
        messages = []
        ...
        for table_name in selected_tables:
            try:
                ...
                data = [tuple(None if isinstance(x, str) and x.strip().upper() == 'NULL' else x for x in row) for row in df.replace({np.nan: None}).values]
...
def home():
    ...
    if request.method == 'POST':
        if 'test_connection' in request.form:
            messages = test_connection(selected_server, selected_username, selected_password)
        if 'fetch_databases' in request.form:
            databases = get_databases(selected_server, selected_username, selected_password)
        if 'fetch_tables' in request.form and selected_database:
            csv_directory = request.form.get('csv_directory', '')
            tables = get_matching_files(csv_directory, get_table_names(selected_server, selected_database, selected_username, selected_password))
        if 'upload' in request.form and selected_database:
            selected_tables = request.form.getlist('tables')
            action = request.form.get('action')
            if 'select_all' in selected_tables:
                selected_tables = get_table_names(selected_server, selected_database, selected_username, selected_password)
            messages = handle_data(selected_server, selected_database, selected_tables, action, selected_username, selected_password)

    return render_template('tanda1.html', databases=databases, tables=tables, messages=messages,
                           selected_server=selected_server, selected_database=selected_database,
                           selected_username=selected_username, selected_password=selected_password)
